...
//reads structures and their details 
...
{ 
  ...
  if(align!=0)
  {
    if(align==1)
    {
      nstruct_new=nstruct_new*2; 
    }
    if(align==2)
    {
      nstruct_new=nstruct*4;
    }
  }
  if(rotation!=0)
  {
...
//adewyer 8-8-18 - alignments
    ...
    if(align!=0)
    {
      if(align==1)
      {
        for(int malign=0;malign<2;malign++)
        {  
      ...
      if(align==2)
      {
        for(int malign=2;malign<6;malign++)
        {  
          for(int mrot=0;mrot<=rotation;mrot++)
          {
            cout << "mrot = " << mrot << endl;
      ...
      if(align==3)
      {
        for(int malign=6;malign<10;malign++)
        {  
          for(int mrot=0;mrot<=rotation;mrot++)
          {
            cout << "mrot = " << mrot << endl;
